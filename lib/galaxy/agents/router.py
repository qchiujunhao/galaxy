...
class QueryRouterAgent(BaseGalaxyAgent):
    ...
    def _create_agent(self) -> Agent[GalaxyAgentDependencies, str]:
        ...
        data_analysis_handoff = self._create_data_analysis_handoff()
    ...
    def _create_data_analysis_handoff(self):
        """Create output function for data analysis handoff.

        This does not execute the data analysis agent directly. Instead it returns a sentinel
        value that `process()` recognizes, so we can run the DataAnalysisAgent with the same
        context (dataset_ids, conversation_history) and preserve structured metadata needed by
        the UI (e.g. pyodide tasks).
        """

        async def hand_off_to_data_analysis(
            ctx: RunContext[GalaxyAgentDependencies],
            request: str,
        ) -> str:
            """Route to data analysis agent for dataset exploration and code-executed analysis.

            Use this when the user is asking to analyze the selected dataset(s) and expects real
            outputs (plots/files/tables) generated by executing Python.

            If no dataset is selected, do NOT use this tool; ask the user to select datasets first.
            """

            # Sentinel that QueryRouterAgent.process understands.
            return f"__HANDOFF_TO_DATA_ANALYSIS__::{request}"

        return hand_off_to_data_analysis
    ...
    async def process(self, query: str, context: Optional[dict[str, Any]] = None) -> AgentResponse:
        ...
        try:
            ...
            if context and context.get("dataset_ids"):
                dataset_ids = [str(ds_id) for ds_id in (context.get("dataset_ids") or [])]
                # Give the router just enough information to know whether datasets are selected,
                # without biasing it toward always handing off to data analysis.
                full_query = (
                    "DATASET SELECTION: "
                    f"{len(dataset_ids)} dataset(s) selected in the UI. "
                    "This is context only; do NOT route to Data Analysis unless the user explicitly asks for "
                    "dataset analysis / executable outputs.\n\n"
                    f"{full_query}"
                )
            else:
                full_query = "DATASET SELECTION: none.\n\n" + full_query
            ...
            try:
                # Special-case: data analysis handoff. We run the data analysis agent here so
                # the UI receives its structured metadata (pyodide_task, artifacts, etc).
                handoff_prefix = "__HANDOFF_TO_DATA_ANALYSIS__::"
                if isinstance(content, str) and content.startswith(handoff_prefix):
                    from .data_analysis import DataAnalysisAgent

                    task = content[len(handoff_prefix) :].strip() or query
